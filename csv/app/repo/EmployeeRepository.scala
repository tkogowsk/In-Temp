...
class EmployeeRepository @Inject() (protected val dbConfigProvider: DatabaseConfigProvider) extends EmployeeTable with HasDatabaseConfigProvider[JdbcProfile] {
  ...
  def insert(employee: Employee): Future[Int] = db.run { empTableQueryInc += employee }

  def insertAll(employees: List[Employee]): Future[Seq[Int]] = db.run { empTableQueryInc ++= employees }

  def update(employee: Employee): Future[Int] = db.run { empTableQuery.filter(_.id === employee.id).update(employee) }
...
private[repo] trait EmployeeTable  { self: HasDatabaseConfigProvider[JdbcProfile] =>
